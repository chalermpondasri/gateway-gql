...
export class CmsRepository implements ICmsRepository {
    ...
    public getMediaContentsByIds(multipleId: number[], opts?: IQueryOptions): Observable<CmsDataResponse<MediaContentDetailResponse>> {
        const filters = {
            id: {
                $in: multipleId,
            },
        ...
        return from(this._axiosInstance.get(`/media-contents`, {
            params:
                {
                    filters,
                    populate: this._mediaContentPopulate,
                    publicationState: opts?.allowDraft ? 'preview': 'live',
                },
        })).pipe(
